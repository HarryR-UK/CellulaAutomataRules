...
void GameOfLife::setSurviveNumberArgs(std::vector<int> temp)
{
    ...
    m_maxSurviveRule = tempMax;
    m_minSurviveRule = tempMin;
...
bool GameOfLife::isAlive(std::array<std::array<int, m_mapSizeY>, m_mapSizeX> &currentMap, const int &x,const int &y)
{
    ...
    if(currentMap[x][y] == 1 && m_neighbourNumbers < m_minSurviveRule)
        return false;

    if(currentMap[x][y] == 1)
    {
        for(std::size_t i = 1; i < m_surviveRules.size(); ++i)
        {
            if(m_neighbourNumbers == m_surviveRules[i])
                return true;
        }
    ...
    if(m_neighbourNumbers > m_maxSurviveRule)
        return false;

    if(currentMap[x][y] == 0)
    {
        for(std::size_t x = 1; x < m_birthRules.size(); ++x)
        {
            if(m_neighbourNumbers == m_birthRules[x])
                return true;
